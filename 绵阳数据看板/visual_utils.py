...
# --- 1. 字体加载逻辑 ---
# 获取当前文件所在目录，并拼接 fonts 文件夹下的 simhei.ttf 路径
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
FONT_PATH = os.path.join(CURRENT_DIR, "fonts", "simhei.ttf")

# 如果字体文件存在，创建 FontProperties 对象，否则使用默认
if os.path.exists(FONT_PATH):
    my_font = fm.FontProperties(fname=FONT_PATH)
else:
    my_font = None
    print(f"⚠️ 警告：未在路径 {FONT_PATH} 找到字体文件，请确认已上传至 GitHub。")

# --- 2. 基础全局样式 ---
sns.set_theme(style="whitegrid", font_scale=1.1)
plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示为方块的问题

# --- 3. 绘图函数定义 (全部注入 fontproperties) ---
...
def plot_special_geo_stacked(df_all_raw, special_list):
    ...
    ax.set_xlabel("区县", fontproperties=my_font)
    ax.set_ylabel("企业数量", fontproperties=my_font)
    for tick in ax.get_xticklabels(): tick.set_fontproperties(my_font)
    if ax.get_legend():
        plt.setp(ax.get_legend().get_texts(), fontproperties=my_font)
...
def plot_region_bar(df, title):
    ...
    ax.set_ylabel("区县", fontproperties=my_font)
    for tick in ax.get_yticklabels(): tick.set_fontproperties(my_font)
    for tick in ax.get_xticklabels(): tick.set_fontproperties(my_font)
    return fig
...
def plot_funding_box(df_sub, col, title):
    ...
    ax.set_xlabel("融资情况", fontproperties=my_font)
    for tick in ax.get_xticklabels(): tick.set_fontproperties(my_font)
    return fig
...
def plot_age_matrix_row(df_sub, col, label):
    ...
    for j, group in enumerate(age_order):
        ...
        if not subset.empty:
            ...
            # 子图标题注入字体
            axes[j].set_title(f"【{group}】均值:{subset[col].mean():.1f}", fontproperties=my_font)
            axes[j].set_xlim(0, 110)
            axes[j].set_xlabel(label, fontproperties=my_font)
            axes[j].set_ylabel("频数", fontproperties=my_font)
    return fig
